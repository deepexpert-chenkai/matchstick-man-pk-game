...
public:
	...
	*/
	static void Init(std::string title = WINDOW_TITLE);
    ...
    */
    static void Draw(SDL_Surface *tex, SDL_Rect &dstRect, SDL_Rect *clip = NULL,
                     float angle = 0.0, int xPivot = 0, int yPivot = 0,
                     SDL_RendererFlip flip = SDL_FLIP_NONE);
	static void Draw1(SDL_Surface *tex, SDL_Rect &dstRect, SDL_Rect *clip = NULL,
                     float angle = 0.0, int xPivot = 0, int yPivot = 0,
                     SDL_RendererFlip flip = SDL_FLIP_NONE);
	static void Drawx(SDL_Surface *tex, SDL_Rect &dstRect, SDL_Rect *clip = NULL,
                     float angle = 0.0, int xPivot = 0, int yPivot = 0,
                     SDL_RendererFlip flip = SDL_FLIP_NONE);
	static void SDL_QuerySurface(SDL_Surface* s,int *w,int *h);
    /**
   ...
   // static void DrawTTF(const std::string &message, const std::string &fontFile,SDL_Color color,int fontSize,int x,int y);
	///Clear the renderer
    static void Clear();
    ...
    static SDL_Rect Box();
	static SDL_Surface *RemoveBackground(SDL_Surface *im,int a,int b,int c);
private:
